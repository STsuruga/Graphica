...
| 素材 | ライセンス | 出典 |
|---|---|---|
| Tabler Icons(`Graphica_project/assets/icons/*.svg`) | MIT | https://tabler.io/icons |

アイコンは SVG のまま同梱し、実行時に色を差し替えて `QIcon` として描画しています
(`Graphica_project/gui/icon_utils.py`)。
...
- アプリ内: ヘルプ ▸ 「Graphica について...」に同じ一覧を表示しています
  (`Graphica_project/gui/dialogs/app.py` の `AboutDialog`)。
