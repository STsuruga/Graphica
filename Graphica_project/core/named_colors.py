...
# ポップアップメニューに直接並べる件数の上限(ユーザー決定)。これを超えたぶんは
# 「すべての登録色...」から検索欄付きの一覧で選ぶ。登録は際限なく増やせるので、
# メニューに全件並べると縦に伸び続けて目的の色を探しにくくなるため。
# 並び順は管理ダイアログの「上へ/下へ」で変えられるので、よく使う色を上位へ
# 持ってくればメニューだけで済む、という想定。
POPUP_LIMIT = 5
