...
4ラウンドの追加対応込み)・H-2-6(#45、ダイアログ群)・H-2-7(#46、
プラグイン管理UIへのスタイル適用)完了。**詳細はこのファイルではなく
...
**H-2-6/H-2-7はフルスイート未実行(2026-08-09時点)**: ユーザーから
「full testは(次の)47まで実施してからでいい」と明示的な指示があったため、
今回はファイル影響範囲のサブセット(`tests/test_dialogs.py`・
`tests/test_color_picker_widget.py`・`tests/test_main_window.py`、
計118件)のグリーン確認のみでコミット・push済み。**H-2-8(#47)着手時、
完了した時点でH-2-6/H-2-7/H-2-8まとめてフルスイートを1回実行すること**
(このセッション内での約束、CLAUDE.mdの通常のRegression bar運用を今回に
限りユーザーの指示で一時的に後ろ倒しにしている)。

**H-2-6/H-2-7で得た知見(今後も踏まえること)**: `gui/dialogs.py`の25
ダイアログをバックグラウンドAgent2体で並行監査した結果、22/25は既存の
...
表見出しがダークモードで見えない)。H-2以降の残り項目でも、「QSSトークンの
調整だけでなく、個別ウィジェットが独自にColorRole/HTMLへ色を直接書き込んで
いないか」を確認する価値がある。
...
ユーザーから明示的に番号(例:「47実施」)で指示があるまで着手しない。
次に来る想定はトラック2 フェーズH-2-8(#47、ステータスバー・通知トースト類。
「通知トースト」は専用ウィジェットではなく`self.statusBar().showMessage(...)`
の一時メッセージを指す、H-2-8着手前に要現状確認)。H-2-5(#44、クイック
アクセスツールバー)はまだ未着手のまま残っている(今回は45/46のみユーザー
から指示された、44は対象外)。H-2は8つのサブ項目(H-2-1〜H-2-8、
ロードマップ#40〜47)のうち#44と#47が残っている。指示が来たらまず
`docs/roadmap.html`の該当行と、`docs/gui_style_audit.md`(H-0の調査結果+
これまでのH-2 Before/After記録)、必要なら
`docs/Graphica_ROADMAP_PLUGIN_AND_GUI.md`のフェーズH節を読んでから着手する。

**H-2-8(#47)が完了したら、H-2-6/H-2-7/H-2-8まとめてフルスイートを実行
すること**(上記「直近の完了」参照、ユーザーの明示的な指示によりH-2-6/
H-2-7はサブセットテストのみでコミット・push済み)。
