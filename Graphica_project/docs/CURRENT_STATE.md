...
- **今回のセッションでroadmap #82(C-101)・#84(C-103)・#91(C-201)・
  ...
  切り替え済み。
- **#146(C-903 スタイルペインター)は着手前に要確認**: 「書式のコピー&
  ペースト」という説明が、2026-08-14に「既存実装確認」で完了扱いにした
  #64(C-902、`gui/mixins/dataset_mixin.py`の`_on_copy_dataset_style`/
  `_on_paste_dataset_style`、右クリックメニュー「スタイルをコピー」/
  「スタイルを貼り付け」)と機能的に重複している。ユーザーに、(a)既存実装で
  充足済みとして完了扱いにするだけか、(b)`STYLE_ATTRS`
  (`gui/mixins/dataset_mixin.py`)が意図的に除外しているグラデーション/
  ウォーターフォール/データ点ラベル/2Dマップ表示方式等をコピー対象に広げる
  拡張実装が必要かを確認してから着手すること。
