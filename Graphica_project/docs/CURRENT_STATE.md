...
**v1.2.1リリース後、トラック3(残りの本体機能追加)3-1(解析基盤)に着手中、ほぼ完了。**
ブランチ`feature/analysis-foundation`で作業中(push済み、PR未作成)。
3-1の10項目のうちC-406(残差プロット)以外の9項目が完了済み(詳細下記
「直近の完了」節、実装の詳細ログは`docs/CORE_FEATURES_PROGRESS.md`参照)。
...
トラック3-1(解析基盤)、10項目中9項目完了(C-406残差プロットのみ残り)。
3回に分けてコミット・push済み(いずれも`feature/analysis-foundation`)。
実装の詳細は`docs/CORE_FEATURES_PROGRESS.md`参照。
...
手順を踏んでから取り込んでいる。roadmap.htmlの#51〜#58・#60を`true`に更新・
re-publish済み(#59 C-406のみ`false`、#150 C-907は表示切替のみのため`false`
のまま、タイトルに注記を追加)。
...
**トラック3-1(解析基盤)の残り1項目、C-406(残差プロット、上下連動2段パネル)
に着手する。** これで3-1(10項目)が完了する。C-406は「フィットのプロット
パネルの下に、残差(実測値-フィット値)を表示する連動パネルを常設する」機能で、
`gui/canvas.py`のサブプロットレイアウトに手を入れる可能性が高い(既存の
`ResultDialog`内の残差プロットはフィット直後に一度だけ表示される非モーダル
ダイアログ内の独立Figureであり、これとは別に「プロットパネル自体に恒常的に
連動表示する」機能だと解釈するのが妥当。着手前にロードマップの原文
`docs/Graphica_CORE_BACKLOG.md`のC-406の説明と、ユーザーへの確認を推奨)。

3-1完了後は、ユーザーに次の作業範囲(トラック3-2以降、またはトラック4の
プラグイン開発)を確認すること。トラック3-2以降に着手する場合は、
`git worktree`で別ディレクトリ・別ブランチを切ってから始めること
(3-1が`gui/canvas.py`・`gui/main_window.py`・`gui/mixins/dataset_mixin.py`
等GUI側の主要ファイルを広く触っているため)。
