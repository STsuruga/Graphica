...
**トラック3-1(解析基盤、全10項目)・トラック3-2(性能・体験、大型2項目を
除く6項目)は共に完了・master統合済み**(PR #3→マージコミット`ce4a13a`、
PR #4→マージコミット`e47d2b1`)。現在のブランチは`master`。
`feature/performance-ux`用に切っていた`git worktree`
(`D:\ユーザー\shuta\ドキュメント\PlotterApp-performance-ux`)はPR #4マージ後に
`git worktree remove`+`git branch -d`で削除済み(リモートブランチも
`gh pr merge --delete-branch`で削除済み)。

**トラック3-2で完了した6項目**: C-1001(表示用ダウンサンプリング/LTTB)・
C-902(一括スタイル適用、既存実装で充足済みと確認)・
C-908(マウス操作拡充: スクロールズーム/中ボタンパン)・
C-909(グラフ上での範囲選択)・C-601(軸共有sharex/sharey)・
C-602(単位変換の第2X軸nm/eV/cm⁻¹/Hz)。実装の詳細ログは
`docs/CORE_FEATURES_PROGRESS.md`の「トラック3-2」節参照。
`docs/roadmap.html`の#63〜68は全て`true`に更新・re-publish済み。

C-601・C-602それぞれの完了直後にフルスイート(`bash scripts/run_tests_chunked.sh`)
を実行し、いずれも実際のテスト失敗ゼロを確認済み(2026-08-14)。C-602のレビュー中、
(1)X軸範囲が0を含む場合に単位変換でinf/nanになりmatplotlibが例外を投げて
再描画全体が失敗する実バグ、(2)新設コンボボックスの長いラベルがQFormLayoutの
共有ラベル列幅を圧迫し既存の横スクロールバー回帰テストを壊す実害、の2件を
発見・修正済み(詳細は`docs/CORE_FEATURES_PROGRESS.md`)。

**次にやること**: 大型2項目(C-003 再描画スコープ分離・C-004 ワーカースレッド
基盤、いずれも★★★アーキテクチャに影響)にどう着手するかをユーザーと相談する
(自主的に着手しないこと)。それ以外にはトラック4(プラグイン本体の開発、
#163〜)にも着手可能。
...
トラック3-2(PR #4)はいずれも既にmasterへマージ・push済み。
`feature/performance-ux`用の`git worktree`(・ローカル/リモート両ブランチ)は
PR #4マージ後に削除済み。次の作業(トラック4、またはC-003/C-004)を始める際は
新しいブランチ/worktreeを切ること。
...
**トラック3-2の6項目(大型2項目除く)は全て完了・master統合済み(PR #4)。**
残っているのは:

- **C-003(再描画スコープ分離)・C-004(ワーカースレッド基盤)は★★★
  アーキテクチャに影響する大型項目のため、ユーザーと相談してから着手する
  (自主的に着手しないこと)。**
- それ以外に着手可能な作業として、トラック4(プラグイン本体の開発、#163〜)
  がある(下記参照)。
