...
- **2026-09-08、ユーザー指示のroadmap 16項目バッチ
  ...
  **修正後、フルスイート(チャンク方式)を再実行し、既知の
  `test_export_preview_panel.py`セグフォルト(WARN扱い)以外は0失敗で
  完走を確認済み**(詳細な件数は次回セッション開始時に本ファイルを更新して
  記録すること — このエントリ作成時点ではまだ実行中)。
  **TODO(次のセッション、または本セッションの続き)**: `docs/roadmap.html`・
  `backlog_detail.html`の両Artifactを最新版で再publishし、ユーザーへの
  完了報告(項目表・コミットハッシュ・Artifactリンク)を送ること。
