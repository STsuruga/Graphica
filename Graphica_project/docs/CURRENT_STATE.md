...
3-1の10項目のうちC-401・C-308・C-907(の一部)が完了済み(詳細下記
「直近の完了」節、実装の詳細ログは`docs/CORE_FEATURES_PROGRESS.md`参照)。
残り: C-403・C-305・C-311・C-411・C-408・C-405・C-406・C-413。
...
トラック3-1(解析基盤): C-401(フィット結果の構造化保持)・C-308(ベースライン
補正: ALS/多項式/ラバーバンド/手動点)・C-907(データセット表示/非表示トグル、
ユーザー追加要望により3-1と同時並行で着手、検索・絞り込み部分は未着手)を
1コミットにまとめて`feature/analysis-foundation`へコミット・push済み
(コミット`65dd727`)。実装の詳細は`docs/CORE_FEATURES_PROGRESS.md`参照。
C-308とC-907はバックグラウンドAgent2体を並列展開して実装し、完了後に差分を
レビューして問題ないことを確認してから取り込んだ。roadmap.html
の#51・#53を`true`に更新・re-publish済み(#150は表示切替のみのため`false`のまま、
タイトルに注記を追加)。
...
**トラック3-1(解析基盤)の残り8項目に着手する**: C-403(パラメータの初期値・
固定・範囲拘束UI)、C-305(共通X格子へのリサンプリング/補間)、C-311(区間積分)、
C-411(ピーク定量表)、C-408(モデルライブラリ拡充)、C-405(信頼帯・予測帯の
描画、C-401完了済みなので着手可能)、C-406(残差プロット)、C-413(フィット
結果の出力、同じくC-401完了済み)。C-403とC-408はどちらも
`core/analysis.py`の`calculate_curve_fit`のディスパッチ部分を触るため、
同時並行(Agent含め)にはせず順番に実施すること。
