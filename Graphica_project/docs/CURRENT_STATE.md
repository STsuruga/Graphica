...
- トラック4(プラグイン本体の開発、#163〜)は`feature/plugin-track4`
  ...
  配布方法をリポジトリ同梱ではなくzip配布+「プラグインをインストール」
  ボタン経由の取り込みに変更する方針が示されたため、進め方の見直しが
  必要(P-805「元素・物理定数テーブル」は実装・テスト済みで
  `feature/plugin-track4`にpush済みだが、配布方法の変更に伴い扱いを
  再検討する必要がある。P-101「JCAMP-DXインポータ」はパーサ部分のみ
  実装、`__init__.py`・テストは未着手のまま停止)。再開する際はまず
  ユーザーに配布方法の方針を確認すること。
