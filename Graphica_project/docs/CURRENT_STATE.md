...
**トラック3-1(解析基盤、全10項目)が完了し、PR #3としてmasterへマージ・
push済み(マージコミット`ce4a13a`、ブランチ`feature/analysis-foundation`は
マージ後に削除済み)。** 現在のブランチは`master`。実装の詳細ログは
`docs/CORE_FEATURES_PROGRESS.md`参照。

マージ前にフルスイート(`bash scripts/run_tests_chunked.sh`)を完走させ、
実際のテスト失敗はゼロだったことを確認済み。その過程で
**`scripts/run_tests_chunked.sh`自体の実バグ**(テストIDをシェル変数に
未クォートで展開していたため、空白/括弧を含むパラメータ化テストID
(今回追加したC-408モデルのfit_type名等)がシェルの単語分割で壊れ、
「file or directory not found: (y」という分かりにくいエラーでチャンクが
偽陽性の失敗になっていた)を発見・修正した(`mapfile`+配列でクォートして
渡す形に変更、コミット`797df48`)。**このスクリプトを使う今後のフルスイート
実行でパラメータ化テストのID起因のエラーが再発することはないはずだが、
念のため「file or directory not found」のようなエラーが出た場合は
テスト内容自体ではなくこのクォート処理を疑うこと。**

**次にやること**: ユーザーとの合意により、トラック3-2(性能・体験、#61〜)の
うち大型2項目(C-003 再描画スコープ分離、C-004 ワーカースレッド基盤、
いずれも★★★アーキテクチャに影響)を除いた6項目
(C-1001・C-902・C-908・C-909・C-601・C-602)に、`git worktree`で
別ディレクトリ・ブランチ`feature/performance-ux`を切って着手する予定
(3-1が`gui/canvas.py`等GUI主要ファイルを広く触っているため、同じ作業ツリーで
の並行作業は避ける方針)。C-003・C-004は規模が異なるため、6項目完了後に
改めてユーザーと相談してから着手する。
...
`feature/analysis-foundation`(トラック3-1用、push済み・PR未作成)。
トラック1(PR #1)・トラック2(PR #2)はどちらも既にmasterへマージ・push済み。
v1.2.1リリース後の機能追加フェーズに入るにあたり、
`docs/Graphica_MASTER_SCHEDULE.md`の「新しいセッション/ブランチを切る単位の目安」
節の推奨(トラック3の区分ごとにブランチを切る)に従ってこのブランチを新設した。
3-1が完了し次第、PRを作成してmasterへマージする(現時点では未実施、
ユーザーとの相談待ち)。
...
**トラック3-1(解析基盤)は完了・master統合済み。** 現在はトラック3-2
(性能・体験)に着手中(またはこれから着手する)。ユーザーとの合意事項:

- 3-2の8項目のうち、まず**大型2項目を除いた6項目**
  (C-1001 表示用ダウンサンプリング・C-902 一括スタイル適用・
  C-908 マウス操作拡充・C-909 グラフ上での範囲選択・C-601 軸共有・
  C-602 単位変換の第2X軸)を先に終わらせる。
- **C-003(再描画スコープ分離)・C-004(ワーカースレッド基盤)は★★★
  アーキテクチャに影響する大型項目のため、上記6項目が終わってから
  改めてユーザーと相談してから着手する(自主的に着手しないこと)。**
- 作業ブランチは`git worktree`で別ディレクトリを切った`feature/performance-ux`
  (masterから分岐。3-1がGUI主要ファイルを広く触っているため、同じ作業ツリーで
  の並行作業は避ける方針)。
