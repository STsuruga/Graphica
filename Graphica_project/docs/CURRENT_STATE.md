...
別ブランチ**`feature/performance-ux`**でトラック3-2(性能・体験)に着手中。

**トラック3-2、大型2項目を除く6項目のうち5項目が完了**:
C-1001(表示用ダウンサンプリング/LTTB)・C-902(一括スタイル適用、既存実装で
充足済みと確認)・C-908(マウス操作拡充: スクロールズーム/中ボタンパン)・
C-909(グラフ上での範囲選択)・**C-601(軸共有sharex/sharey、直近完了)**。
実装の詳細ログは`docs/CORE_FEATURES_PROGRESS.md`の「トラック3-2」節参照。

**残りは C-602(単位変換の第2X軸)のみ。** C-003(再描画スコープ分離)・
C-004(ワーカースレッド基盤)は★★★アーキテクチャに影響する大型項目のため、
C-602完了後に改めてユーザーと相談してから着手する(自主的に着手しないこと)。

C-601完了直後にフルスイート(`bash scripts/run_tests_chunked.sh`、
`feature/performance-ux`ワークツリー側)を実行し、実際のテスト失敗ゼロを
確認済み(2026-08-14)。`docs/roadmap.html`の#67は`true`に更新・
re-publish済み(masterブランチ側、コミット`514be86`)。
`feature/performance-ux`側の実装コミットは`80599ac`。
...
**トラック3-2、残りは C-602(単位変換の第2X軸)のみ。** 完了したら:

1. `feature/performance-ux`側でフルスイートをもう一度(またはC-602の
   変更が小さければ影響範囲に応じたサブセットで)確認。
2. `docs/CORE_FEATURES_PROGRESS.md`・`docs/roadmap.html`(#68)を更新。
3. 3-1と同じ手順(PR作成→即マージ→ブランチ削除)でmasterへ統合、pushまで。
4. **C-003(再描画スコープ分離)・C-004(ワーカースレッド基盤)は★★★
   アーキテクチャに影響する大型項目のため、上記完了後に改めてユーザーと
   相談してから着手する(自主的に着手しないこと)。**
