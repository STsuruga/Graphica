...
- **2026-09-08、コードベース全体の監査を実施し、改善項目20件を「Graphica 改善ボード」
  ...
  **項目IDで指示すればそのまま作業指示として使える**(例:「改善ボードのA-1をやって」)。
  ...
  確認できる。**未着手の重要項目**:
  - **A-1(重大)**: ウォーターフォール有効時、範囲選択マスク・データカーソル・
    データエディタ連動ハイライト・ピーク配置の4箇所が表示座標→データ座標の
    逆変換をしておらず、積み重ね2本目以降で操作が全てずれる。
  ...
  **ユーザー判断が済んだ項目(2026-09-08)**:
  - **A-4**: ウォーターフォールで非表示にしたトレースの分を繰り上げず、
    **位置を保持する(隙間が空く)** 方針に決定。積み重ねインデックスの採番を
    `visible`フィルタの前に行い、描画時のみ非表示を除外する
    (`redraw_all`/`update_single_axis`の3経路すべてで揃えること)。
