...
## 次にやること(2026-08-29時点、TODO)

- **最優先**: v1.3.5(タブ埋め込み順序修正)がMacで実際に直っているか、
  ユーザーからの実機確認待ち。あわせてv1.3.4で対応した3件
...
- **C-913(グラフ書式の既定値設定)**: `docs/Graphica_CORE_BACKLOG.md`の
  ...
  選択肢を提示)。
- 「追加機能の開発」を次のチャットで行うとユーザーから予告あり。具体的な
  機能は次のチャットで指定される見込み(2026-08-29時点で未確定)。
  C-913がその候補の一つだが、他の可能性もある。
