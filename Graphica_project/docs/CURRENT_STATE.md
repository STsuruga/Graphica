...
未マージ・未削除**)は一時停止中だったが、**2026-09-08の調査で停止理由は
既に解消済みであることを確認した**(改善ボードのD-4参照):

- **配布機構は完成済み**: `core/plugin_install.py`(zip展開・マニフェスト検証・
  パストラバーサル対策、テスト付き)、環境設定の「プラグインをインストール...」
  ボタン(zip選択)、`%LOCALAPPDATA%\Graphica\plugins`
  (`core/app_paths.py`の`get_user_plugins_dir()`)。停止理由だった
  「zip配布+インストールボタン」がそのまま実装されている。
- **本体(master)に中途半端なプラグイン実装は残っていない**。
  `plugins/example_plugin/`は意図的な同梱サンプル。P-101(JCAMP-DX)の
  「パーサのみ実装」はどのブランチにも痕跡が無く、未着手として扱ってよい。
- **未マージの成果物が1件**: P-805(元素・物理定数テーブル、実装+テスト計496行)が
  `feature/plugin-track4`に1コミット(`30313e4`)だけ載っている。masterは
  そこから82コミット進んでいるが、`plugin.json`は現行の必須キー
  (name/version/api_version="1.0")を満たしておりAPI互換性は保たれている。

**再開手順**: (1)ブランチをmasterにrebaseしてテストを通す、(2)P-805を
「同梱サンプル」にするか「配布用zip」にするかを決める、(3)その後
P-402(統計検定、ユーザー判断によりプラグインとして実装する方針)以降に着手。
