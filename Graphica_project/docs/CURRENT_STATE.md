...
**v1.2.1リリース後に着手したトラック3-1(解析基盤)が全10項目完了した。**
ブランチ`feature/analysis-foundation`で作業(push済み、PR未作成、まだmasterへ
マージしていない)。実装の詳細ログは`docs/CORE_FEATURES_PROGRESS.md`参照。

**次のセッションでまず判断すること**: PRを作成してmasterへマージするか、
それとも先に次のトラック(3-2以降、またはトラック4)に進めるか、ユーザーに
確認する(このセッションでは確認できていない)。マージする場合は、
「push前には一定の頻度でフルスイートを挟む」という通常方針に対し、
このセッションでは**ユーザーの判断でC-406着手時点の最終フルスイートを
意図的に省略した**(理由: 直前の個別確認(test_residual_panel.py全パス・
test_dataset_mixin.py -k residual全パス・test_main_window.pyを26%まで実行し
全パス確認済みの時点で時間コストとの兼ね合いにより中断)。**マージ前に
一度はフルスイートを完走させることを推奨する**(`bash
scripts/run_tests_chunked.sh`、cwdは`Graphica_project/`、2〜3時間程度)。
...
**トラック3-1(解析基盤)は完了した。** ユーザーに次の進め方を確認すること:
1. `feature/analysis-foundation`のPRを作成してmasterへマージするか
   (推奨: マージ前に一度フルスイートを完走させる。上の「現在の状況」節参照)。
2. 次に着手するトラック(3-2「性能・体験」#61〜、3-3「差別化機能」、
   トラック4「プラグイン本体」#163〜のいずれか)。

トラック3-2以降に着手する場合は、`git worktree`で別ディレクトリ・別ブランチ
(例: `feature/performance-ux`)を切ってから始めること(3-1が`gui/canvas.py`・
`gui/main_window.py`・`gui/mixins/dataset_mixin.py`等GUI側の主要ファイルを
広く触っており、3-2の項目の多くも同じファイルを触るため、同じ作業ツリーで
並行するとファイル競合・実行中テストへの干渉が起きる。このセッション中にも
実際にユーザーから並行着手の提案があり、この理由で見送っている)。
