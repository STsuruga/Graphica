...
**v1.2.1リリース後、トラック3(残りの本体機能追加)3-1(解析基盤)に着手中。**
ブランチ`feature/analysis-foundation`で作業中(push済み、PR未作成)。
3-1の10項目のうちC-401・C-403・C-308・C-311・C-411・C-907(の一部)が
完了済み(詳細下記「直近の完了」節、実装の詳細ログは
`docs/CORE_FEATURES_PROGRESS.md`参照)。残り: C-305・C-408・C-405・C-406・C-413。

**背景Agentの成果物は必ず`git diff`で内容を読んでからテストを自分で実行して
確認すること(自己申告のサマリーだけを信用しない)。** このセッションで
実際に2回、Agentの成果物に対する検証が必要になった実例がある:
...
2. 別のバックグラウンドAgentが、指示(「フルスイートは実行しない」)に反して
   素の`pytest -q`(全件・チャンク分割なし)を実行し、2時間以上ハングしたまま
   放置されていたプロセスが見つかった(単発pytestの既知のハング問題、
   下記「既知の注意点」参照)。Agentへの指示に「フルスイートを実行しない」と
   明記していても、実際に守られるとは限らないため、作業完了後は
   `Get-Process`等でその名残がないか確認する習慣が有効。
...
トラック3-1(解析基盤): 2回に分けてコミット・push済み(いずれも
`feature/analysis-foundation`)。実装の詳細は`docs/CORE_FEATURES_PROGRESS.md`参照。
...
- 2回目(コミット`b6bc2af`): C-403(パラメータの初期値・固定・範囲拘束UI)・
  C-311(区間積分)・C-411(ピーク定量表)。3件ともバックグラウンドAgentを
  並列展開して実装し、完了後に差分をレビュー・自分でテストを再実行して
  1件のテストの不備(`QMessageBox.information`のモック漏れによるヘッドレス
  環境でのハング)を発見・修正してから取り込んだ。

いずれもバックグラウンドAgent(C-308・C-907は2体並列、C-403・C-311・C-411は
3体並列)で実装し、完了後に差分レビュー→自分でテスト再実行→問題があれば
修正、という手順を踏んでから取り込んでいる。roadmap.htmlの#51・#52・#53・
#55・#56を`true`に更新・re-publish済み(#150 C-907は表示切替のみのため`false`
のまま、タイトルに注記を追加)。
...
**トラック3-1(解析基盤)の残り5項目に着手する**: C-305(共通X格子への
リサンプリング/補間)、C-408(モデルライブラリ拡充、Voigt/pseudo-Voigt/
Lorentzian等。`core/analysis.py`の`calculate_curve_fit`と`get_fit_param_names`
両方に新しいelif節を追加する必要がある点に注意)、C-405(信頼帯・予測帯の
描画、C-401完了済みなので着手可能)、C-406(残差プロット)、C-413(フィット
結果の出力、同じくC-401完了済み)。
