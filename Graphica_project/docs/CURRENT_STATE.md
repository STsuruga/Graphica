...
## 現在の状況(2026-08-14時点、最優先で読むこと)

**トラック3-1(解析基盤、全10項目)は完了・master統合済み**(PR #3、
マージコミット`ce4a13a`)。現在は`git worktree`で切った別ディレクトリ
(`D:\ユーザー\shuta\ドキュメント\PlotterApp-performance-ux\Graphica_project`)・
別ブランチ**`feature/performance-ux`**でトラック3-2(性能・体験)に取り組んでいる。

**トラック3-2、大型2項目(C-003・C-004)を除く6項目が全て完了**:
C-1001(表示用ダウンサンプリング/LTTB)・C-902(一括スタイル適用、既存実装で
充足済みと確認)・C-908(マウス操作拡充: スクロールズーム/中ボタンパン)・
C-909(グラフ上での範囲選択)・C-601(軸共有sharex/sharey)・
**C-602(単位変換の第2X軸nm/eV/cm⁻¹/Hz、直近完了)**。
実装の詳細ログは`docs/CORE_FEATURES_PROGRESS.md`の「トラック3-2」節参照。
`docs/roadmap.html`の#63〜68は全て`true`に更新・re-publish済み
(masterブランチ側、直近コミット`00fd611`)。`feature/performance-ux`側の
実装コミットは直近`42b89ea`(C-602)。

C-601・C-602それぞれの完了直後にフルスイート(`bash scripts/run_tests_chunked.sh`、
`feature/performance-ux`ワークツリー側)を実行し、いずれも実際のテスト失敗ゼロを
確認済み(2026-08-14)。C-602のレビュー中、(1)X軸範囲が0を含む場合に単位変換で
inf/nanになりmatplotlibが例外を投げて再描画全体が失敗する実バグ、(2)新設コンボ
ボックスの長いラベルがQFormLayoutの共有ラベル列幅を圧迫し既存の横スクロールバー
回帰テストを壊す実害、の2件を発見・修正済み(詳細は`docs/CORE_FEATURES_PROGRESS.md`)。

**次にやること(要ユーザー確認)**: 3-1の時と同じ手順(PR作成→即マージ
→ブランチ削除)で`feature/performance-ux`をmasterへ統合するかどうか、
および大型2項目(C-003 再描画スコープ分離・C-004 ワーカースレッド基盤、
いずれも★★★アーキテクチャに影響)にどう着手するかは、まだユーザーの
指示を仰いでいない(3-1の時の「マージ関連は提案の通りに」はその場限りの
承認であり、3-2に対する標準的な合意事項ではないため、自主的にPR作成・
マージまでは進めていない)。

過去の`scripts/run_tests_chunked.sh`自体の実バグ(テストIDの未クォート展開)
は3-1完了時に既に修正・解消済み(詳細は下記「既知の注意点」節)。

**背景Agentの成果物は必ず`git diff`で内容を読んでからテストを自分で実行して
確認すること(自己申告のサマリーだけを信用しない)。** このセッションで
実際に複数回、Agentの成果物に対する検証が必要になった実例がある:
1. C-403のテスト1件が、他の同種テストと違って`QMessageBox.information`の
   モックを忘れており、ヘッドレス環境で本物のモーダルダイアログが
   `.exec()`でブロックし続けてテストプロセスが完了しなくなっていた
   (CPU使用率0%のまま無限に「実行中」に見える。`ps`ではWindowsネイティブ
   プロセスが見えないことがあるため、PowerShellの`Get-Process`でCPU時間の
   増分がゼロであることを確認して初めてハングと判明した)。
2. 背景Agentが、指示(「フルスイートは実行しない」)に反して素の`pytest -q`
   (全件・チャンク分割なし)、あるいは`pytest tests/test_dataset_mixin.py -q`
   (ファイル単位でもチャンク分割なし、この重いファイルは単体でもハングしうる)
   を実行し、放置されたまま何時間もハングし続けるプロセスが**複数回
   (このセッションだけで3件)** 見つかった。**Agentへの指示文に「フルスイートを
   実行しない」と明記していても、実際に守られるとは限らない。** Agentが完了
   報告した直後は毎回、`Get-Process | Where-Object { $_.ProcessName -like
   "*python*" }`(必要なら`Get-CimInstance Win32_Process -Filter
   "ProcessId=<PID>" | Select CommandLine`でコマンドラインを確認)で放置
   プロセスがないか確認し、見つかったら`Stop-Process -Id <PID> -Force`で
   個別に停止する習慣が有効(生きているかどうかはCPU時間の増分で判定、
   下記「既知の注意点」参照)。

**ユーザーとの運用合意(このセッションで決定、以降も踏襲):**
- フルスイートの実行回数は最小限にする。3-1全10項目を通して、
  「core/配下に触れる最初のまとまった変更が済んだ時点」と「3-1完了・push直前」
  の**合計2回**に抑える方針(ユーザーの明示的な指示)。個々の項目ごとには
  フルスイートを挟まず、変更したファイルに対する`pytest -k <キーワード>`
  サブセットのみで進める。
- ファイルの重なりが少ない項目は、背景Agentを断りなく並列展開してよい
  (既存の合意を踏襲)。ただし**Agentは共有ツリー上で並行編集するため、
  同じファイルに複数Agent/自分の変更が同時に入ると、実行中のフルスイートが
  「走行中に内容が変わるファイルを対象にしてしまい結果が無効になる」ことが
  実際に起きた(C-401検証中にC-308/C-907のAgentが同じcore/analysis.py等を
  編集し始め、走行中だったフルスイートの結果が使えなくなった)。フルスイートを
  流す前に、並行編集中のAgentが完全に終わっているか確認すること。**
...
`feature/performance-ux`(トラック3-2用、`git worktree`で
`D:\ユーザー\shuta\ドキュメント\PlotterApp-performance-ux\Graphica_project`に
別ディレクトリとして切ってある。push済み・PR未作成)。
トラック1(PR #1)・トラック2(PR #2)・トラック3-1(PR #3)はいずれも
既にmasterへマージ・push済み。C-602完了後、PRを作成してmasterへマージする
(3-1と同じ手順を踏襲予定、現時点では未実施)。
...
**トラック3-2の6項目(大型2項目除く)は全て完了済み。** 残っているのは:

1. `feature/performance-ux`をmasterへ統合するかどうかユーザーに確認する
   (3-1と同じPR作成→即マージ→ブランチ削除の手順で進めてよいか、
   タイミングを含めて指示を仰ぐ。pushまでは既に許可されているが、
   PR作成・master統合は都度確認する運用としている)。
2. **C-003(再描画スコープ分離)・C-004(ワーカースレッド基盤)は★★★
   アーキテクチャに影響する大型項目のため、ユーザーと相談してから着手する
   (自主的に着手しないこと)。**
...
- トラック4(プラグイン本体の開発、#163〜)はトラック1完了により着手可能
  ...
  トラック3-2以降に着手する場合は、`git worktree`で別ディレクトリ・別ブランチ
  (例: `feature/performance-ux`)を切ってから始めること(3-1が
  `gui/canvas.py`・`gui/main_window.py`・`gui/mixins/dataset_mixin.py`等
  GUI側の主要ファイルを広く触っているため、同じ作業ツリーで3-2を並行すると
  ファイル競合のリスクが高い)。
- フルスイートの実行回数は上の「現在の状況」節の合意通り最小限にする
  (3-1完了・push直前にもう1回のみ)。
