...
## 現在地(2026-09-26): M2(R-2・R-4・R-5)完了、次は K-31 のあと M3
...
- **M0・M1(R-0)完了**(PR #25): 特性テスト `tests/characterization/`。conftest が QSettings を一時 INI に向け、差し替え忘れのモーダルは例外にする。
- **M2 完了**: R-2 フィットのモデルの表(#26)、R-4 アプリの設定の表 `gui/app_settings.py`(#31)、R-5 通知の窓口 `gui/notify.py`(#34)。
  K-20(#27)・K-6(#28)・K-30(#29)・K-25(#30)・K-24(#33)も完了。
- 特性テストの数値は、基準を作った機械(`golden/MACHINE.json`、この PC)ではビット単位、CI などほかの機械では許容差で比べる(#32、ユーザー選択)。
  GitHub の Windows ランナーは CPU が毎回違い、最後のビットが揺れたため。
- 作業中の PR: K-32(CI のジョブに時間の上限、#35)、K-17(利用者に見える文言から項目番号を取る)。
- 次: K-31(測定情報つきの TXT を読むと落ちる。数値の表の自動検出・TXT にも「列の型を確認」と最大行数・文字の列の警告、ユーザー選択)を
  `fix/k31-text-import` で直す → M3(R-3 データセットの処理を「操作の定義+実行役」に、R-6 データセットの並びを 1 つのクラスに)。
- まだ直さない K(時期つき): K-27・K-9(R-8)、K-28・K-29・K-19(R-1)、K-5・K-18(R-6)、K-21・K-8(R-3)、K-7(R-7)、ほか merge。
...
- F-2 完了(`0828189`): `except Exception` 72か所を仕分け。利用者に表示するだけだった17か所にも traceback を
  ログに残す、黙って握りつぶしていた箇所は例外の種類を絞るかログを残す。
...
- H はすべて完了・取り込み済み。
- I-2 完了、PR #21 で master に取り込み済み(マージコミット ea77f57): 起動コマンドを gui-scripts に(コンソールを開かない)。依存は範囲指定
  ...
  動かない)を登録。
- I-3 完了、PR #22 で master に取り込み済み(マージコミット 945b7ef): 配布名は graphica-plot(ユーザー決定。PyPI の graphica は別人のもの。import 名と
  起動コマンドは graphica のまま)。v* タグの push で .github/workflows/publish.yml が TestPyPI に上げ、そこから
  graphica-plot だけを取って起動を確かめてから本番 PyPI に上げる(Trusted Publishing、トークン不要)。
  **まだ一度も公開していない**: PyPI/TestPyPI のアカウント作成と pending publisher の登録、GitHub の環境
...
- **ついでに見つかったアプリの改善も同じブランチで直す**(ユーザー指示)。ボードの「K」
  (db の `findings` コレクション、doc_id `K-n`)に登録してから直す。ユーザーもボードのフォームから追加できる。
  挙動を変える修正はリファクタリングのコミットに混ぜず、1件1コミット(`fix:`)でテストを先に書く。
  仕様判断が要るもの・M 以上は確認してから。詳しいルールはボードの「作業ルールと引き継ぎ」。
...
| 項目 | 対応 |
|---|---|
| Ctrl+Shift+S 重複 | 名前を付けて保存だけに。エクスポートはショートカットなし。全アクションのショートカット一意性をテスト |
| 保存確認 | `ProjectModel.content_fingerprint()`(保存内容のハッシュ、active_axis_index除外)で判定。タブを閉じる/終了/プロジェクトを開く(メニュー・最近使ったファイル)で「保存/保存しない/キャンセル」。**テストは conftest の `GRAPHICA_CONFIRM_UNSAVED_CHANGES=0` で無効化**(モーダルでハングするため) |
| .txt/.xls | `gui/workers.py` の `BUILTIN_DATA_FILE_EXTENSIONS` に一本化、xlrd 追加。区切り文字をワーカー側でも推測(タブ区切りCSVが2列チェックで弾かれていたバグも修正)。`tests/fixtures/legacy_two_sheets.xls` |
| データ点ラベル上限 | 実測 約2.5ms/件・再描画ごと(1,000件2.5秒、20,000件68秒)→ 安定性を担保できないので確認ダイアログ廃止・非表示のまま理由をパネルに表示 |
| フィット曲線 solid 表示 | `core.dataset.linestyle_name` で '--'/'dashed' を正規化(リストのアイコンも同じ不具合だった) |
| 標準偏差 | `core.analysis.sample_standard_deviation`(ddof=1)に統一。要約の「σ」→「SD」。Z-score も n−1 |
| 凡例ドラッグ | **そもそもドラッグ不能だった**(matplotlib 3.x に無い `draggable()` を握りつぶし)。`set_draggable(update='loc')` + `legend_position` 保存 |
| パレット管理 | 組み込みパレットも初期選択対象に |
| ミニマップ | 有用性判断: ピーク位置の点はズーム先の目安として有用、かつ散布図全般が線で結ばれる不具合だったため、点系の種別を点で描く修正にした(除外はしない) |
| プレビューPDF | `gui/export_settings.export_rc_params` に全経路を一本化 |
| README/CLAUDE.md | 現行UIに合わせて全面更新、Wiki へのリンク。環境設定のオートセーブ既定保存先表示も修正 |

リリース前のカバレッジ付きフルスイート: 109チャンク・3,074 passed・失敗0(既知の test_export_preview_panel WARN のみ)。
| 行カバレッジ | **93.4%** (14,302 / 15,109 行) |
| 分岐カバレッジ | 88.6% (3,499 / 3,948) |

### 以前の状況(2026-09-16)

**v1.4.1(目盛線の長さ設定)をリリース**: 目盛線の長さ設定。
`major_tick_length`(pt、既定3.5)/`minor_tick_length`(pt、負値=自動=主×2/3.5)。
軸サイズに対する倍率ではなく pt の絶対値にしたのはユーザー判断(文字・線も pt 固定で、
目盛りだけ相対にすると大サイズ出力で比率が崩れ、多パネル図で長さが揃わない)。
「大サイズ出力で全体が小さくなる」対策のエクスポート一括スケーリングは検討したが
今回は不要とされた。テスト: `tests/test_tick_length.py`。
リリース前にカバレッジ付きフルスイート: 101チャンク・2,938 passed・失敗0(既知WARNのみ)、
行93.4%/分岐88.6%。カバレッジの詳細(全モジュールの数字と未到達の行番号)を
`docs/COVERAGE_DETAILS.md` として新たにコミットするようにした。
...
- **★ macOS の CI が2回連続で落ちていたのを発見・修正した**(Windowsジョブは通って
  いたため手元では見えなかった)。原因は C-1 の実機フィードバック対応で
  **見出しの文字サイズを px で固定した**こと。このアプリはQSSでフォントサイズを
  指定しておらず、フォームのラベルはOS既定で描かれる — その既定が
  **Windows 9pt / macOS 13pt前後**と差が大きいため、13px固定の見出しは macOS では
  「見出し13px < 項目15px」と逆転し、ユーザーがWindowsで指摘したのと同じ症状に
  なっていた。`theme.heading_point_sizes()` を新設し、**アプリ既定フォントからの
  相対(本文+1pt / +2pt)**で決めるよう変更。テスト側も「QSSに書いた数値」と
...
- **2026-09-08、コードベース全体の監査を実施し、改善項目20件を「Graphica 改善ボード」
  (Artifact: https://claude.ai/code/artifact/947b83ab-3d04-406f-91ca-824ac03d5513 )に
  まとめた**。カテゴリはA:バグ(6件)/B:アーキテクチャ(4件)/C:GUI導線(4件)/
  D:新機能提案(3件)/E:性能(3件)。各項目に根拠(ファイル:行)と対応方針を記載しており、
  **項目IDで指示すればそのまま作業指示として使える**(例:「改善ボードのA-1をやって」)。
  進捗はArtifactのdb機能に保存されるため、別セッションからも
  `read_db`(collection `status`、doc idが項目ID、body `{state: todo|doing|done}`)で
  確認できる。**未着手の重要項目**:
  **完了済み(2026-09-08)**:
  - **C-1(データセットのプロパティパネルのサブセクション化)**:
    `formLayout_4`(Designer生成分+実行時追加の**39行が1本**)を、
    `DATASET_PROPERTY_SECTIONS`(`gui/main_window.py`)で定義した**7つの
    折りたたみ可能なサブセクション**に分割した。
    - **着手前に実測した根拠**: 表示行数はプロット種別によって
      1D Scatter 20行(パネル高763px)〜2Dグリッド 30行(1134px)まで動き、
      **トグルひとつで最大371px伸縮**していた。ボードの「41行」は当時の値で、
      A-5以降の実測は39行。
    - **区分(ユーザー決定、案Aを採用)**: データ列(5)/基本スタイル(10)/
      グラデーション(3)/ウォーターフォール(6)/2Dマップ・値による配色(9)/
      表示の追加要素(3)/配置・情報(3)。ボード原案の7区分は
      **凡例名・平滑化2行・データ点ラベル2行・フィット情報の計6行がどこにも
      入らない**ことが精査で判明したため、そこを埋めた案に差し替えた
      (GUIモックアップを提示して選んでもらった)。
    - **折りたたみの既定(ユーザー決定)**: **全展開 + 開閉状態をQSettingsに記憶**。
      保存するのは「**閉じている**セクションのキーのJSON配列」なので、
      キーが無い/空=全展開。新しいセクションを足しても移行処理が要らない。
    - **番号指定の insertRow(N, ...) はこのパネルから完全に消えた**。従来は
      行1〜5の挿入が「この順に実行されること」に依存する地雷だった
      (CLAUDE.mdが警告していた当該箇所)。以降の追加は
      `self._prop_form(<キー>).addRow(...)` で末尾に積むだけでよい。
    - **中身が全部隠れたセクションは見出しごと隠す**
      (`_update_property_section_visibility()`)。でないと折りたたみで
      減らしたぶんを見出しが食い返す。C-2の「選択状態で空になるサブメニューは
      出さない」と同じ方針。`dataset_mixin.py`の4つの
      `_update_*_controls_visibility()`の末尾(ウォーターフォールの早期return
      分岐も含む)から呼ぶ。
    - **移設は `takeRow()` で行う(`removeRow()` はウィジェットごと破棄するので
      使ってはいけない)**。Designer生成の「平滑化」チェックボックスだけは、
      後から追加される「平滑化の手法」と隣接させるため意図的に移設を保留し、
      手法コンボの直前で足している(そのまま移すと間に「透明度」が挟まる)。
      「フィット情報」も同様に、非表示100pxの欄が先頭に来ないよう
      セクション末尾へ回した。
    - **併せて D-2 でフラグしたまま残っていた潜在問題を解消**:
      プラグインが `register_plot_type()` で登録する名前は任意長のため、
      長い名前でプロパティドックが横にはみ出す恐れがあった。
      `plot_type_combo` に `AdjustToMinimumContentsLengthWithIcon` +
      `setMinimumContentsLength(16)` を設定して根本対処した
      (ベースライン画像の取り直しがC-1と共通なので同じ回にまとめた、
      ユーザー承認済み)。
    - **ベースライン画像2枚(`main_window_light.png`/`main_window_dark.png`)を
      取り直した**。見出し7本ぶんパネルが約133px縦に伸びるため
      (全展開時)、意図した見た目の変更。
    - サブセクションの見出しは `objectName="property_subsection_toggle"`
    ...
    - **★ 実機フィードバックで3件追加修正(いずれも同日)**:
      ...
      3. **見出しと項目の区別がつきにくい**。当初は「外側のアコーディオンより
         ...
         引かれない点に注意。
      4. **インデントが逆転**。実測すると親の見出しは`padding-left:4px`、子は`0`で、
         **子のほうが4px左**に出ていた。親(4px)→子(12px)→中身(24px)の
         階段にした。**`QToolButton`の既定は「文字幅ぴったり」**なので
         `setSizePolicy(Expanding, Fixed)`が無いと罫線が文字の下までしか
         引かれない(これも実地で踏んだ)。
      5. **見出しが中の項目より小さい**。`QFontMetrics`で実測したところ、
         子見出し12px・**項目ラベルも実高さ12px**(9pt)で、太さしか違わなかった。
         親14px > 子13px > 項目12px の階段にした。
      6. **ホバー時の文字色が白で消えていた**(私の実装ミス)。`accent_text`は
         ライトモードで`#FFFFFF`(アクセント**背景に載せる**文字色)なので、
         薄い背景のホバーに使うと見えなくなる。`selection_accent`へ。
    - テスト: `tests/test_property_sections.py` を新設(57件)。

- **B-2(`gui/dialogs.py` の分割)、2026-09-15完了**:
  5,560行・47ダイアログの1ファイルを、機能群ごとの**6モジュール**へ分割し
  `gui/dialogs/` パッケージにした。粒度はユーザー決定(1ダイアログ1ファイルの
  47分割ではなく、機能群でまとめる案を採用)。
  | モジュール | 行数 | 担当 |
  |---|---|---|
  | `data_import` | 719 | 取り込み・列の型・新規データセット |
  | `data_edit` | 715 | 列の計算/文字列操作・行フィルタ・重複X |
  | `analysis` | 1,690 | フィット・ピーク・ベースライン・積分・規格化・外れ値 |
  | `export` | 528 | 画像/一括エクスポート・キャプション・色覚シミュレーション |
  | `appearance` | 1,036 | ラベル書式・配色パレット・登録色・凡例順・矢印・インセット |
  | `app` | 1,160 | 環境設定・ヘルプ・ようこそ・コマンドパレット・プラグイン引数 |
  - **呼び出し側44箇所は1行も変えていない**。すべて
    `from gui.dialogs import X` の形だったので、`__init__.py` の再エクスポートで
    吸収できた。
  - **着手前の調査で分割の安全性を確認済み**: クラス間参照は4箇所だけ
    (`ColumnPreview→ColumnType`、`MultiPeakFit→PeakSettings`、登録色の2つ→
    `_named_color_icon`)で、いずれも同じモジュールに収めたので**循環importは無い**。
    モジュールレベルの定義も`logger`と`_named_color_icon`のみだった。
  - **★ 分割で1件だけ壊れた**: `tests/test_dialogs.py` が
    `monkeypatch.setattr("gui.dialogs.QDesktopServices.openUrl", ...)` と、
    **ダイアログが取り込んだ名前をフラットなモジュール上で差し替えて**いた。
    パッケージ化後は `gui.dialogs.app` が持ち主なので、そちらを指すよう修正。
    パッケージ側はクラスを再エクスポートするだけなので、そこを差し替えても
    ダイアログからは見えない。
  - **`pyproject.toml` の packages に `gui.dialogs` を追加**。ここは明示列挙
    なので、足し忘れると `pip install -e Graphica_project` した外部プラグイン
    リポジトリから見えなくなる。
  - テスト: `tests/test_dialogs_package.py` を新設(16件)。**再エクスポートの
    網羅性**(足し忘れは遅延importのせいで「その画面を開いた瞬間」まで分からない)、
    **サブモジュール間importの禁止**、**1モジュールが再肥大化しないこと**、
    **呼び出し側がサブモジュールを直接参照し始めていないこと**を固定している。
  - フルスイート100チャンク・2,920 passed・0失敗(既知のWARNのみ)、17分10秒。

- **E-3(テストスイートの実行時間)、2026-09-15完了。約35分 → 約17.5分**:
  `pytest --durations` で上位を潰す作業のつもりで着手したが、**根本原因は
  個々のテストではなくスイート全体の衛生状態**だった。
  - **★ 根本原因: テストが閉じずに残したウィンドウが溜まり、後ろのテストほど
    遅くなっていた**。`test_main_window.py` の165件を通す間に**生存ウィジェットが
    16,707 → 98,525 個まで線形に増加**。効くのは「全ウィジェットを走査する処理」で、
    テーマ再適用(`app.setStyleSheet()` は生きている全ウィジェットを再ポリッシュ
    する)やアイコンの一括更新がそれ。**同じテストが単独 0.79秒 → 165件中では
    66秒**になっていた。
    `tests/conftest.py` に autouse の `destroy_leftover_windows` を追加して解決。
    **`close()` だけでは足りない** — Qtは`close()`してもC++オブジェクトが生き続け、
    `deleteLater()`の予約もイベントループを回すまで実行されないため、
    `QApplication.sendPostedEvents(None, QEvent.Type.DeferredDelete)` まで行う
    (実測: `close()`のみ 349秒→304秒、破棄まで **349秒→74秒**)。
    **conftestのautouseフィクスチャはテスト側のフィクスチャより先にセットアップ
    される=後片付けは後から走る**ので、自前で`close()`しているテストと衝突しない。
  - **収集を1プロセスに集約**。`run_tests_chunked.sh` はファイルごとに
    `pytest --collect-only` を起動しており、92ファイル×約2.8秒=**約255秒**を
    「テストを1件も実行しないまま」消費していた(一括なら5.3秒)。
    収集はテスト本体を実行しないので、チャンク分割の理由である蓄積は起きない。
    まとめての収集がそのファイルを落とした場合の保険として、単体収集は残してある。
  - **`test_settings_mixin.py` 234秒 → 31秒**。`_mutate_every_control` が約90個の
    コントロールを順に書き換えるたびに `_on_axis_setting_changed` →
    `_update_plot_appearance()`(tight_layout + draw)が走っていた。
    ラウンドトリップの検査対象は収集/適用であって途中の再描画ではないので、
    **プロジェクトへの保存は残したまま再描画だけ止める**(`_without_live_redraw`)。
  - **`CHUNK_SIZE` 30 → 150**。細かく切っていたのは蓄積対策だったが、その主因を
    断ったので大きいファイルは1プロセスの方が速い(`test_dataset_mixin.py` 435件:
    30件×15チャンク 308秒 → 150件×3チャンク 237秒)。チャンク数 150 → 98。
    **ファイル単位の分離自体は残す** — `test_export_preview_panel.py` の
    「全件パス後に終了処理でセグフォルト」があり、1プロセスにまとめると以降が
    道連れになるため。
  - テスト: `tests/test_suite_hygiene.py` を新設(8件)。**ここが壊れても
    「テストが失敗する」のではなく「スイートがじわじわ遅くなる」だけ**なので、
    後始末が効いていること・収集が1プロセスであること・再描画停止が外れて
    いないことを明示的に固定している。
  - フルスイート99チャンク・2,904 passed・0失敗(既知のWARNのみ)、**17分33秒**。

- **E-2(注釈の全削除→全再生成を、内容が変わっていなければ省く)、2026-09-15完了**:
  `MplCanvas._annotation_render_keys`(Axesごとの内容キー)を新設し、
  `_draw_annotations(..., allow_reuse=True)` のときだけ前回と比較して
  作り直しを丸ごと省く。
  - **再利用してよいのは `update_appearance_only()` だけ**。調べた結果、
    注釈を描く4経路のうち他の3つは Artist が既に死んでいる:
    `redraw_all`=`fig.clf()`、`_redraw_single_axis_no_draw`=`ax.cla()`、
    `add_free_axis`=新規Axes。**既定を `allow_reuse=False` にして**、将来の
    呼び出し元が何も考えずに安全側へ倒れるようにした(取り違えると注釈が消える)。
  - **キーには「結果に影響する入力」を全部入れる**(グローバル規約):
    注釈リストに加えて (1)**ダークモード** — テキスト/矢印の色は
    `_effective_text_color()` を通るので、これを落とすと「ダークモードにしたのに
    注釈の色だけ元のまま」になる、(2)**統計値アンカーラベルの確定テキスト** —
    この経路が `datasets` を受け取る唯一の理由が値の再計算なので、計算結果の
    文字列そのものをキーに入れる、(3)**インセットが参照するデータセットの
    色・線幅・不透明度・表示状態**。
  - **インセットが描くデータ本体はキーに含めない**。`update_appearance_only` は
    `_draw_data()` を呼ばない=「データは変わっていない」を前提に本体の描画も
    省いている経路なので、インセットだけ別の前提を置く必要がない
    (データが変わる操作は `ax.cla()` を伴う別経路を通る)。
  - **再利用しない経路でもキーは更新する**。しないと全体再描画の直後の1回が
    必ず無駄に描き直しになる。
  - 実測(20万点、`_draw_annotations` 1回あたりの中央値):
    インセット1個 **8.6ms → 0.01ms**、2個 14.8ms → 0.03ms、4個 **41.8ms → 0.03ms**。
    軸の書式スピンボックスを1つ動かすたびに走る経路なので、体感に直結する
    (外観更新全体に占める割合は、20万点+インセット1個で約8〜15%。
    `tight_layout()`と`draw()`が支配的なため、インセットが増えるほど効く)。
  - テスト: `tests/test_annotation_reuse.py` を新設(16件)。速度ではなく
    **「再利用してはいけない場面で再利用していないこと」**を主に固めている。
    フルスイート150チャンク・2,896 passed・0失敗(既知のWARNのみ)。

- **2026-09-15、新機能「よく使う色の名前付き登録」を実装**(ボード/ロードマップ
  外のユーザー要望)。用途は「複数の種類のデータで、同じ物質には同じ色を
  使いたい」。GUIモックアップを提示し、着手前に3点をユーザー決定:
  **保存先=QSettings(全プロジェクト共通)/一括適用も今回入れる/登録名の重複は禁止**。
  - `core/named_colors.py` を新設(GUI・Qt非依存の純ロジック)。キーは
    `named_colors_json`、**配列**で保存する(ユーザーの並び順がそのまま
    ポップアップの表示順になるため、辞書にして順序を失ってはいけない)。
    **既存の配色パレット(`custom_color_palettes_json`)とは別キー**。あちらは
    「系列に順番に割り当てる**色のリスト**」、こちらは「**1名前1色**」で別物。
    同居させるとパレット管理側の意味が壊れる。
    色の検証は`QColor`ではなく正規表現で行い、`QApplication`なしで単体テスト
    できるようにしてある。壊れた登録は**1件ずつ捨てて残りは読む**
    (設定ファイルが壊れたときに起動できなくなるより、登録が減る方がマシ)。
  - `gui/color_picker_widget.py`: スウォッチを押すと従来は直接`QColorDialog`が
    開いていたのを、**登録色の一覧+「この色を登録...」「色名の管理...」
    「その他の色...」のポップアップ**に変更(元々ツールチップが
    「クリックしてパレットを開く」なので挙動としても素直)。このウィジェットは
    **データセットの色とグラデーション終端色の両方**で使われているので、
    1箇所の実装で両方に効く。
  - `gui/dialogs.py`に`NamedColorManagerDialog`を追加(追加/編集/削除/並べ替え)。
    **OKを待たずその場で保存する**。ここでの操作は「登録簿を育てる」行為で
    プロットの見た目を変えないため、Cancelで巻き戻す必要が薄い
    (パレット管理側は「どれをアクティブにするか」の選択を伴うのでOK/Cancelを持つ、
    という違い)。
  - オーバーフローメニュー(⋯)に「登録した色を適用 ▶」を追加。N件への適用は
    `beginMacro`でまとめて**Undo 1回**で戻る(`_on_auto_assign_colors`と同じ型)。
    **★ 実装中に踏んだ罠**: 対象を絞らずに`beginMacro`/`endMacro`すると、
    Qtは**中身ゼロのマクロをそのままスタックへ積む**ため、「既にその色」の
    データセットしか選んでいないと「押しても何も起きないUndo」が1個増える。
    先に変更対象を絞ってから開始するよう修正し、回帰テストを付けた。
  - **登録は際限なく増やせるので、ポップアップに並べるのは先頭5件まで**
    (`POPUP_LIMIT`、ユーザー決定)。溢れたぶんは「すべての登録色... (N件)」から
    `NamedColorPickerDialog`(名前・色コードで絞り込める一覧)で選ぶ。
    色欄のポップアップと「登録した色を適用 ▶」の両方に同じ上限が効く。
    よく使う色は管理ダイアログの「上へ」で上位に持ってくればメニューだけで済む。
  - テスト: `tests/test_named_colors.py`を新設(55件)。
  - **2026-09-15、C-1とこの新機能をまとめてフルスイート検証済み**:
    149チャンク・2,880 passed・0失敗(既知の`test_export_preview_panel.py`
    終了時セグフォルトのみWARN)。コミット`db22e59`〜`f2f2140`をpush済み。

- **2026-09-14、実機フィードバックによるUI調整(C-1と同じ回で対応)**:
  - **押下フィードバックの色を青に統一**(「どのタブも押したときの色が緑だから
    青で統一して」)。ティール系`accent_soft`が残っていた4箇所
    (アコーディオン見出し×2・スピンボックスの上下ボタン・タブ追加ボタン)と、
    `accent`のままだったスプリッタのつまみを`selection_highlight`/
    `selection_accent`へ。**ティール系が残るのは操作フィードバックではない表現
    (`QProgressBar::chunk`)だけ**になった。
    `tests/test_theme.py`に、**トークン名ではなく展開後の実際の色**で
    「`:hover`/`:pressed`/`:selected`/`:checked`/`:focus`のどこにもティールが
    無い」を見る網羅テストを追加(将来トークンを経由せず直接書いても検出できる)。
  - **スクロールバーのハイライトは濃いグレー(`text_muted`)へ方針転換**。
    ここは実機フィードバックで**2度変わっている**: 1回目「緑のままだから青で統一」で
    `accent`→`selection_accent`、2回目「スクロールバーは青にハイライトよりも
    濃いグレーでハイライトがいいかも」で`text_muted`。スクロールバーは
    「今どこを選んでいるか」を示す選択表現ではなく掴む場所を示す操作部品、という
    整理。`text_muted`は通常時の`border_strong`に対しライトでは濃く・ダークでは
    明るくなるので、どちらのモードでもコントラストが上がる。
    既存テスト(青であることを固定していた)は削除せず、経緯を書いて書き換えた。
  - **ツールバーに「データセット操作」ボタンを追加**(「適当な名前でツールバーからも
    アクセスできるようにしたらいい気がする」)。C-2で整理した右クリックメニューは
    中身が豊富なのに右クリックでしか辿り着けず気づかれにくいため。
    - `_on_dataset_tree_context_menu()`から中身の構築を
      **`_populate_dataset_actions_menu(menu)`へ切り出して両導線で共有**する
      (片方だけに項目を足す壊れ方を防ぐ。テストで両者の一致を固定した)。
    - 右クリック側は毎回新しい`QMenu`を渡す使い捨て、ツールバー側は永続`QMenu`を
      `aboutToShow`のたびに`clear()`して詰め直す(選択状態で出し入れされる項目が
      あるため)。サブメニューのPython参照は`menu._graphica_submenus`に持たせる
      (PySide6がサブメニューを回収する既知の癖への保険)。
      **連続して開いても壊れないこと**を回帰テストで固定。
    - テスト: `tests/test_menu_restructure.py`に4件追加(計28件)。
      「どのセクションにも属さない迷子ウィジェットが無いこと」を直接見る
      テストを入れてある(takeRowしたまま足し忘れると、削除も警告もされず
      グループボックスの左上に重なって描画されるため)。
  - **B-4(専用テストの無い大きめのファイルにテストを追加)**:
    `tests/test_settings_mixin.py`(24件)と`tests/test_plugin_manifest.py`(21件)を
    新設。`load_plugin_manifest`は直接呼ぶテストが1件も無かった(間接的に通って
    いるだけ)ので全分岐を固めた。
    - **★ テスト整備中に実バグを1件発見・修正した**:
      `_apply_settings_to_ui_controls`(保存値の**復元**)が末尾で
      `_on_x_autoscale_changed()`(ユーザーがチェックを**外した瞬間**用に、
      現在表示中の軸範囲をスピンボックスへシードするハンドラ)を呼んでいたため、
      **オートスケールOFFで保存した軸範囲が、復元のたびに「いま画面に出ている
      範囲」で上書きされて失われていた**。手動で軸範囲を指定したプロジェクトを
      開き直す/サブプロットを切り替えるたびに起きる。
      実測では`x_min/x_max = 111.0/222.0` → `-0.05/0.05`。
      **X側だけが壊れY側は無事**という順序依存の壊れ方だった(先に走るX側の
      ハンドラが再描画を起こすが、その時点で軸にはまだ復元前の範囲が入っている。
      Y側はその再描画で既に復元後の値が軸へ反映済みなのでシードしても値が
      変わらなかった)。
      値を書き換えない`_refresh_x/y_autoscale_enabled_state()`を切り出し、
      復元経路はそちらを呼ぶよう修正。ユーザー操作時のシード(実機フィードバック
      由来の既存のバグ修正)は温存し、両方に回帰テストを付けた。
    - **C-1を意識した「自動で効き続ける」テストを2本入れてある**:
      `test_every_gathered_key_is_consumed_by_apply`(収集したキーを適用側が
      読んでいなければ落ちる。`inspect`でソースを検査するので将来キーを足しても
      自動で効く)と
      `test_changing_any_control_reaches_the_project_settings`(42ウィジェットを
      操作して値がプロジェクトまで届くか確認。**C-1でパネルを組み直して再配線を
      忘れたらここで落ちる**)。
    - `ui_setup_mixin.py`はC-2/C-3/C-4で新設した`tests/test_menu_restructure.py`
      (24件)がメニュー構築と`_collect_menu_actions`をカバーしており、
      B-4として追加のテストは書いていない。
  - **D-1(カテゴリ列による系列の自動分割、long形式データの取り込み)**:
    `core/analysis.py`に`split_dataframe_by_column()`を新設(純粋関数)し、
    コンテキストメニューの「データ処理」サブメニューに「列の値で系列に分割...」を
    追加した(行フィルタを条件の数だけ手作業で繰り返す代わりの操作なのでその隣)。
    - **グループの順序はファイル中の初出順**(pandasのgroupbyの既定である値の
      ソート順ではない)。測定順・濃度順のようにファイルの並び自体に意味がある
      ことが多く、そのまま保たれるほうが予測しやすいため。
    - 分割列が欠損(NaN)の行は除外し、**その件数をステータスバーに出す**
      (黙って減らすと「点が足りない」と気づきにくい)。
    - **元のデータセットは残す(非破壊)**。不要なら削除すればよく、削除も
      A-3でUndo可能になっている。
    - **色はアクティブなパレットから順に割り当てる**。`Dataset.color`の既定値は
      固定の`'#1f77b4'`なので、そのままだと全系列が同じ色になり分割した意味が
      ほとんど無くなるため(グラデーションにしたい場合は追加後にC-805の
      「カラーマップから自動配色」を掛ければよい)。
    - 追加は`AddDatasetCommand`経由で`beginMacro`にまとめ、**N件の追加が
      Undo 1回で戻る**。
    - 連続値の列を誤って選ぶと大量の系列ができるため、
      `SPLIT_BY_COLUMN_CONFIRM_THRESHOLD`(30)を超える場合は作る前に確認を挟む。
    - テスト: `tests/test_split_by_column.py`を新設(21件)。
      C-2で固定していた「データ処理」サブメニューの期待値も1項目追加で更新。
  - **C-2 / C-3 / C-4 / A-5(メニュー導線の整理、まとめて実施)**:
    - **C-2**: データセット右クリックメニュー(約30項目がほぼフラット)を
      「データ処理(9)/解析・注釈(4)/複数データセット(5)/エクスポート(3)/
      タブ操作(2)」の5サブメニューに整理。**トップレベルは12項目**まで減った
      (残したのはフォルダ操作・スタイルのコピー/貼り付け・再読み込み・削除など
      「1クリックで終わる頻出操作」)。選択状態によって中身が空になるサブメニュー
      (1件選択時の「複数データセット」等)は出さない。
      このメニューは右クリックのたびに作り直す使い捨てなので、メニューバー側と
      違い`self.xxx`への永続参照は不要(ローカルの`submenus`リストで保持)。
    - **C-3**: ファイルメニューのエクスポート系7項目を`エクスポート ▶`へ集約し、
      **22項目→15項目**に。色覚シミュレーションプレビューは「出力」ではなく
      「確認」の機能なので表示メニューへ移した。サブメニューは`QMenu`と
      `menuAction()`の**両方**を永続参照(`_export_menu`/`_export_menu_action`)。
    - **C-4**: ファイルメニュー先頭に「データファイルを開く(&D)...」を追加し、
      中央の「データ追加」ボタンと同じ`_on_add_dataset`へ繋いだ。**ショートカットは
      割り当てない**(Ctrl+Oは既に「プロジェクトを開く」が使っており、動かすと
      既存ユーザーの手癖を壊すため)。
    - **A-5**: 2Dマップ(`data_kind='2d_grid'`)ではウォーターフォール設定を
      チェックボックスごと隠す(描画の手前で1D経路から分離されるため何の効果も
      無い)。選択を2D→1Dへ戻すと再表示されることもテスト済み。
    - **★ 併せて対応: クイックアクセスのピン留め識別子の移行**。ピンの識別子は
      メニューの**パス全体**(`"ファイル(F) > 名前を付けてエクスポート(S)..."`)で、
      `_restore_quick_access_actions()`は一致しない識別子を**黙ってスキップする**
      設計。そのままC-3をやると既存ユーザーのピンがエラーも警告も無く消える。
      `_migrate_pinned_quick_access_ids()`を新設し、「末尾の項目名が一致する候補が
      現在のメニューにちょうど1つだけある」場合に読み替えて、結果をQSettingsへ
      書き戻す(同名候補が複数ある場合は誤爆を避けて触らない)。
    - **テスト側の対応**: `tests/test_dataset_mixin.py`の`_RecordingMenu`が
      サブメニュー化で壊れる(サブメニューも同じ差し替えクラスで作られるため
      `last_instance`がサブメニューを指してしまう)ので、第1引数が文字列かどうかで
      ルートとサブメニューを見分け、`added_texts`/`actions_by_text`は
      「自分+配下」を平坦化して返すように変更。これで既存16件のコンテキストメニュー
      テストは**1行も書き換えずに通る**。
    - テスト: `tests/test_menu_restructure.py`を新設(24件)。
  ...
  - **A-1 + A-4(同時実施)**: `MplCanvas`に積み重ね変換の記録
    ...
    `display_to_data()` / `data_to_display()` / `get_waterfall_transform()`
    を新設。スカラーもnumpy配列も受け、ウォーターフォール無効/未描画の
    データセットに対しては恒等変換になる。**A-1の4箇所**
    (範囲選択マスク・データカーソル・エディタ連動ハイライト・ピーク配置)を
    すべてこのAPI経由に変更した。
    - ピーク配置だけは扱いが2系統に分かれる: 初期値(`_pending_peak_guesses`)は
      フィット計算が生の`x_data`/`y_data`と突き合わせるため**データ座標**で保持し、
      仮マーカーはユーザーがクリックした**表示座標**に描く。右クリック削除の
      最近傍判定も、guessではなくマーカーArtistの実描画位置から読むように変更
      (でないと常に見当違いのマーカーが選ばれる)。
    - カテゴリX軸では描画側がXシフトを行わないため、`offset_x`は0.0として記録する
      (記録しないとXだけ二重にずれる)。
    - **A-4**: `visible`フィルタを3つの呼び出し側から`_draw_data()`の内部へ移し、
      積み重ねインデックスの採番だけを「非表示も含むリスト」で行うようにした。
      これで`redraw_all`/`update_single_axis`/`add_free_axis`の3経路すべてで
      同じ採番になり、非表示にしたトレースの段は空いたまま保持される
      (ユーザー判断どおり)。ベースラインとzorderの刻み幅も同じリストから求める。
    - **副作用として1件、既存の不整合を解消した**: `_draw_annotations()`へ渡す
      データセットのリストが、`redraw_all`だけフィルタ済み・他2経路は非フィルタ
      という不整合があり、統計値アンカーラベル(C-708)が「非表示にしただけの
      データセット」に対して経路によって`(データセットなし)`と表示されたり
      実際の値を表示したりしていた(エクスポートは`redraw_all`経由なので、
      非表示のデータセットに紐づくラベルが`(データセットなし)`で出力されていた)。
      3経路とも非フィルタに揃え、実際の値を表示する側に統一した。
    - テスト: `tests/test_waterfall_coordinates.py`(変換の単体・16件)と
      `tests/test_waterfall_interaction.py`(実際のGUI経路4箇所の回帰・10件)を新設。
  - **A-2 + B-1(同時実施)**: 7つの排他マウスモードの排他制御を
    `gui/mixins/mouse_mode_mixin.py`(新規、`MOUSE_MODES`登録簿 +
    `_deactivate_other_mouse_modes()`)に集約し、各mixinの手書きif連鎖
    (7ファイル×6分岐=42分岐)を1行の呼び出しに置き換えた(-124行/+43行)。
    **ボードにはA-2として「注釈モードだけが自由配置編集モードを解除しない」と
    記録していたが、実際は`CursorMixin`にも同じ抜けがあり、解除漏れは2件だった**
    (置換スクリプトが「解除していたモード数」を数えた結果、cursorとannotationだけが
    5、他5モードは6だったことで確定)。集約により両方が同時に解消。
    `tests/test_mouse_modes.py`を新設し、**42通りの全順序対**で
    「Xを有効にするとYが解除される(フラグ・ツールバーのチェック状態の両方)」を
    検証する(49 passed)。B-1はボード記載の「独立した`MouseModeManager`を新設して
    `activate(name)`に全部寄せる」案ではなく、**登録簿を持つ`MouseModeMixin`を
    `PlotterApp`の基底に追加する軽量版**を採用した(QActionの`triggered`接続を
    張り替えずに済み、「8つ目の追加＝登録簿に1行」という目的は同じだけ達成できる)。
    `CLAUDE.md`のmixin数(14→15)と「手書き42分岐」の記述も更新済み。
    **注意**: QActionは`toggled`ではなく`triggered`に接続されているため、
    `setChecked(False)`だけではスロットが再入しない。解除は「チェックを外す」と
    「`_toggle_*_mode(False)`を呼ぶ」の2段構えが必須(登録簿側で両方行っている)。

  **ユーザー判断が済んだ項目(2026-09-08)**:
  - **D-3**: 統計検定(P-402)は**プラグインとして実装する**方針に決定。
    コア機能への昇格は行わない。
  **改善ボードは D-3(プラグイン開発、ユーザー判断で保留)を除き全項目完了。**
  D-3(統計検定プラグイン)はユーザー判断で保留中。

- **2026-09-08、ユーザー指示のroadmap 16項目バッチ
  ...
  (一括表示切替/密度散布図)・`4be79eb`(画像回帰は既存実装確認)・`b9698f4`
  (CVDシミュレータ)・`119a4d2`(レポート自動生成)・`f824eaa`(ドックレイアウト
  保存/読込/リセット)・`2bd8ff4`(インセット拡大図)・`507c262`(ピーク自動
  ラベル、衝突回避)・`4b8695d`(範囲選択のブリッティング高速化)・`c98ca60`
  (ProjectModelのシグナル化、最小スコープ、着手前にユーザーへ選択肢提示・
  承認を得てから実装)。
  **#80(C-005)は元のバックログの狙い通りの「全箇所シグナル化」フル
  refactorではなく、最小スコープ版(QObject化+`changed`シグナル+
  `notify_changed()`のみ追加、既存の`_update_plot()`直接呼び出し約38箇所は
  無変更)を選択**(コスト・リスクの大きい設計判断のため、着手前にユーザーに
  選択肢を提示し承認を得た)。
  **バッチ完了後の最終フルスイート(チャンク方式)で実際のリグレッションを
  1件発見・修正**: #152(C-911、ドックレイアウトのメニュー追加)が原因で、
  `_collect_menu_actions()`(コマンドパレット/クイックアクセス管理/
  ショートカット一覧が使う)を一度でも呼ぶと「ドックレイアウト」サブメニュー
  本体がPySide6側で破棄され、以後そのメニューにアクセスするたびに
  `RuntimeError: Internal C++ object already deleted`でクラッシュする不具合
  (`tests/test_quick_access_mixin.py`の3件で顕在化)。根本原因は、
  サブメニュー自体(`self._dock_layout_menu`)を永続参照として保持するだけでは
  不十分で、そのサブメニューを開くための`QAction`(`QMenu.menuAction()`)側も
  別途永続参照として保持しないと、`view_menu.actions()`を辿る一時参照が
  失われた際にPySide6がメニュー本体ごと破棄してしまう(実測確認、既存の
  「トップレベルメニューはmenuBar().actions()経由ではなくキャッシュ済み
  ...
  **修正後、フルスイート(チャンク方式、133チャンク)を再実行し、既知の
  ...
  最新版でrepublish済み、ユーザーへの完了報告(項目表・コミットハッシュ・
  Artifactリンク)も送信済み。**この16項目バッチは完全に完了。**
...
- **2026-09-03、roadmap #85(C-104)・#88(C-107)・#90(C-109)・#93(C-203)・
  ...
  マスクしない)。フルスイート(157チャンク)完走・0失敗を確認し`48e97b9`で
  コミット済み(push試行中)。
...
- **2026-09-03、ユーザー指示によりroadmap #100/#107/#111/#114/#116/#118/#121/
  ...
  (それぞれ`LabelEditDialog`のmathtextライブプレビュー+記号パレット、
  データセットリストの既存フォルダ機能)で満たされていることを確認し完了扱い
  とした(新規コード変更なし)。roadmap.html/backlog_detail.htmlは republish
  済み。
...
- **今回のセッションでroadmap #82(C-101)・#84(C-103)・#91(C-201)・
  ...
  #102/#103/#110/#124/#130は`888eec6`でpush済み)。フルスイート
  (チャンク方式、117チャンク)は0失敗・既知のtest_export_preview_panel.py
  セグフォルトのみWARNで完走確認済み。
...
- **#153(C-912 スタートアップ画面)は意図的にスコープを絞った**: 起動の
  ...
  ため)、既存の初回ウェルカムダイアログを拡張しつつ、「ファイル」メニューの
  「スタートアップ画面...」からオンデマンドで開ける形にとどめた。自動表示化を
  望む場合は別途相談してから着手すること。
- **#130(C-703 矢印のバリエーション拡張)実装完了により、「実装方向で検討中」
  節に残るのはC-704(スケールバー)のみ**(下記「実装方向で検討中」節参照)。
...
### トラック4(プラグイン本体開発)は再開済み・プラグインは別リポジトリ(2026-09-13、改善ボード D-4)
...
- **P-805(元素・物理定数テーブル)は別リポジトリへ移設済み**:
  https://github.com/STsuruga/graphica-plugin-element-constants
  一旦masterへマージ(`71e4f3c`)したが、方針転換に伴いrevertして切り出した。
  移設先ではテスト16件が通っている(本体を`pip install -e`した状態。
  本体が無い環境ではデータ検索の12件だけが走り、残り4件はskip)。
  本体リポジトリの`plugins/element_constants/`と
  `tests/test_element_constants_plugin.py`は削除済み。
- **本体が`pip install -e`できることが前提になった**。`Graphica_project/pyproject.toml`
  が`core`/`gui`/`gui.mixins`/`models`をトップレベルパッケージとして公開している
  ため、外部リポジトリからでも`from core.plugin_testing import FakeGraphicaPluginAPI`
  が使える(CLAUDE.mdは「pyproject.tomlは無い」と書いていたので訂正済み)。
- **配布はzipのみ**。`plugin_search_paths()`は`if not is_frozen()`のときだけ
  本体の`plugins/`を探索するため、**外部リポジトリのプラグインを利用者へ届ける
  経路はzipインストールだけ**。各プラグインリポジトリが`scripts/build_zip.py`を
  持ち、作ったzipが実際に`install_plugin_zip()`で入ることをテストで固めてある。
  本体側にも`scripts/build_plugin_zip.py`(`example_plugin`用)を追加した。
- **★ ボードの記述に誤りがあった**: 「P-101(JCAMP-DX)の『パーサのみ実装』は
  どのブランチにも痕跡が無い」と記録していたが、これはブランチだけを見ていた
  ためで、**worktreeに未コミットのまま`plugins/jcamp_dx_importer/parser.py`
  (221行)が残っていた**。このセッションで放置worktreeを2つ削除しており、
  確認せずに消していれば失われていた。`feature/plugin-track4`に
  WIPコミット(`e6f5a7e`)として退避・pushしてある。
  **まだ動くプラグインではない**(`plugin.json`も`__init__.py`も無いため
  PluginManagerは認識しない)。P-101着手時はここから取り出して
  `graphica-plugin-jcamp-dx`(仮)へ移すこと。

**ブランチの現状**: `feature/plugin-track4`(worktree
`PlotterApp-plugins`)にはP-101のWIPだけが残っている。**削除しないこと。**
...
- **改善ボード(コード監査TODO、2026-09-08〜)での作業指示**:
  ...
  指示してくる(例:「改善ボードのA-1をやって」)。各項目には根拠(ファイル:行)と
  対応方針が書かれており、**そのまま作業指示として使う**。
  - **着手前**に必ず`read_db`(`action: "read_db"`, `db_op: "list"`,
    `collection: "status"`)で完了済み項目を確認する。docのidが項目ID、
    bodyは`{state: "todo"|"doing"|"done", note, updatedAt}`。
  - **1項目の実装が完了するごとに**`write_db`(`db_op: "set"`,
    `collection: "status"`, `doc_id: <項目ID>`)で`state: "done"`と
    `note`(コミットハッシュ+要約)を書き込み、**更新後のボード進捗を
    ユーザーに表示する**(ユーザーからの明示指示、2026-09-08)。
  - ボードのHTMLソース(`ITEMS`配列)はArtifact側にのみ存在し、リポジトリには
    無い。項目の追加・記述変更が必要な場合は`action: "read"`でHTMLを取得して
    編集・republishする。状態(`status`)はHTMLではなくdb側が正。
...
- **★ フルスイートを2本同時に走らせないこと(2026-09-13に実際に事故を起こした)。**
  `scripts/run_tests_chunked.sh`はチャンクファイルを`.ci_test_chunks/`という
  **固定のディレクトリ**に作るため、複数の実行がこれを共有する。片方を
  `TaskStop`等で止めると、その終了処理が`.ci_test_chunks/`を空にし、
  **走り続けているもう片方が参照するチャンクファイルまで道連れになる**。
  そうなるとスクリプトは
  `line 47: .../test_dataset_mixin_chunk_ab: No such file or directory`を出し、
  結果として`python -m pytest -q`(**ファイル引数なしの素のpytest**)を起動して
  しまう。これはCLAUDE.mdが禁じている全件一括実行そのもので、
  **約2時間・メモリ7.6GBまで膨らんで一切完了しなかった**。
  別の実行を始める前に、前の実行が完全に終わっていることを必ず確認する。
  事故った場合は、bash/pythonのプロセスを全て停止 →`.ci_test_chunks/`を削除 →
  クリーンな状態で再実行すること。

- **★ スタック判定にCPU時間の差分「だけ」を使わないこと(同じく2026-09-13)。**
  上記の事故のとき、CPU時間は20秒間で19.9秒増えており(=1コアを100%使用)、
  それだけ見て「健全に計算中」と誤判定した。実際にはログの最終更新が
  **2時間前**で止まっていた。壊れたまま回り続けるケースがあるため、
  **必ず「ログファイルの最終更新からの経過秒数」も併せて見る**こと。
  進捗報告にも含める(例: `LOG_IDLE_SEC=45`)。

- **★ フルスイート実行中の30分ごとの進捗報告は、仕組みで担保すること
  (同じく2026-09-13にユーザーから指摘を受けた)。** CLAUDE.mdに明記されている
  にもかかわらず、バックグラウンドの完了通知を待つだけで途中報告をしていなかった。
  スイート起動と同時に、`sleep 1800`してから進捗スナップショット
  (チャンク数/全体数/パーセント/passed/failed/LOG_IDLE_SEC)を出力する
  バックグラウンドタスクも起動し、それが終了するたびに報告して再セットする。
