...
- K(ついでの改善、ボード db の `findings`): K-1(set_cell の pandas 3 対応, `06e380d`)・K-2(読み込み失敗時に
  前のファイルが上書き保存先に残る, `e56587d`)を修正済み。K-3〜K-11 は登録のみ。
  **ユーザーの返事待ち**: K-4(ピーク閾値の既定を「なし」にする案A推奨)、K-11(復元直後のタブ名の表記、
  「無題のプロジェクト(復元)」推奨)、K-5(内容の確認)。
