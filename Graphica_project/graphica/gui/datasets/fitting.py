"""曲線フィット(単発・一括・多峰分離)と、フィット結果の再表示・注釈への焼き込み。"""
import numpy as np
import pandas as pd
...
class FittingController:
    ...
    def __init__(self, host):
        ...
        self.batch_fit_runner = None
        self.multi_peak_fit_runner = None

    def shutdown(self):
        """タブを閉じる前に、実行中の計算を待って片付ける(curve_fit 自体は中断できない)。"""
        for name in ("fit_runner", "batch_fit_runner", "multi_peak_fit_runner"):
            runner = getattr(self, name)
            if runner is not None:
                _stop_runner(runner)
                setattr(self, name, None)

    def _show_result(self, title, text, csv_data, residual_x, residual_y):
        if self.result_dialog is not None:
            self.result_dialog.close()
        self.result_dialog = ResultDialog(title, text, self._host.parent_widget, csv_data=csv_data,
    ...
    def fit_current_dataset(self):
        ...
        (fit_type, custom_formula, use_weighted, x_range,
         p0_overrides, fixed_params, bounds, band_type, loss) = FitDialog.get_fit_type(
            self._host.parent_widget, x_min=x_min, x_max=x_max
        )
        if fit_type is None:
            return
        ...
        runner.succeeded.connect(lambda fit: self._on_fit_succeeded(
            source, fit_type, custom_formula, sigma, x_range, p0_overrides, fixed_params, bounds, band_type, loss, fit,
        ))
        runner.failed.connect(self._on_fit_failed)
        self.fit_runner = runner
        self._host.set_fit_button_enabled(False)
        runner.start()

    def _finish_fit_runner(self):
        if self.fit_runner is not None:
            self.fit_runner.wait()
            self.fit_runner.deleteLater()
            self.fit_runner = None
        self._host.set_fit_button_enabled(True)
    ...
    def _on_fit_succeeded(self, source, fit_type, custom_formula, sigma, x_range,
        ...
        self._host.add_derived_dataset(dataset, source)
        csv_data = pd.DataFrame({
            'パラメータ': list(fit['param_names']) + ['R^2'],
            '値': list(fit['popt']) + [fit['r_squared']],
        })
        # 残差は NaN の除外とフィット範囲の適用後の点なので、x もそれに合わせる
        self._show_result("フィッティング完了", text, csv_data, fit['x_data_used'], fit['residuals'])
    ...
    def batch_fit_selected(self):
        ...
        runner.succeeded.connect(lambda results: self._on_batch_succeeded(results, target_folder, progress))
        runner.failed.connect(lambda msg: self._on_batch_failed(msg, progress))
        self.batch_fit_runner = runner
        runner.start()

    def _finish_batch_runner(self, progress):
        if self.batch_fit_runner is not None:
            self.batch_fit_runner.wait()
            self.batch_fit_runner.deleteLater()
            self.batch_fit_runner = None
        progress.close()
    ...
    def _on_batch_succeeded(self, results, target_folder, progress):
        ...
        if added:
            self._host.add_datasets_to_folder(added, target_folder)
    ...
    def multi_peak_fit_current_dataset(self):
        ...
        if source is None:
            return
        if self.multi_peak_fit_runner is not None:
            self._busy()
            return
        x_data, y_data = source.x_data, source.y_data
        component_type, baseline_type, initial_guesses = MultiPeakFitDialog.get_multi_peak_fit_settings(
            self._host.parent_widget, x_data=x_data, y_data=y_data,
            initial_guesses=self._host.pending_peak_guesses(),
        )
        self._host.finish_peak_placement()
        if component_type is None:
            return

        runner = TaskRunner(multi_peak_fit_task, x_data, y_data, component_type, initial_guesses,
                            baseline_type=baseline_type)
        runner.succeeded.connect(lambda fit: self._on_multi_peak_fit_succeeded(source, fit))
        runner.failed.connect(self._on_multi_peak_fit_failed)
        self.multi_peak_fit_runner = runner
        self._host.set_multi_peak_fit_button_enabled(False)
        runner.start()

    def _finish_multi_peak_runner(self):
        if self.multi_peak_fit_runner is not None:
            self.multi_peak_fit_runner.wait()
            self.multi_peak_fit_runner.deleteLater()
            self.multi_peak_fit_runner = None
        self._host.set_multi_peak_fit_button_enabled(True)
    ...
    def _on_multi_peak_fit_succeeded(self, source, fit):
        ...
        self._show_result("多峰分離フィット完了", text, csv_data, fit['x_data_used'], fit['residuals'])
    ...
    def show_fit_result(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            self.burn_fit_result_annotation(dataset, fit_result)
    ...
    def burn_fit_result_annotation(self, dataset, fit_result):
        ...
        self._host.show_status("フィット結果を注釈として焼き込みました")
